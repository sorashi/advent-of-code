...
def cold_cache_hyperfine(input_filename):
    ...
    subprocess.run(["hyperfine", "--input", input_filename, "--export-json", tmpfile, "--prepare", "sync; echo 3 | tee /proc/sys/vm/drop_caches", binary_path], capture_output=True, text=True)
    return extract_data(tmpfile)

def warm_cache_hyperfine(input_filename):
    tmpfile = tempfile.mktemp()
    subprocess.run(["hyperfine", "--export-json", tmpfile, "--input", input_filename, "-N", "-w", "5", binary_path], capture_output=True, text=True)
    return extract_data(tmpfile)
...
def main():
    ...
    build_release()
    print(f"# {day}\n")
    print("| description | download | CRC32 | size | lines | silver | gold | CCM [ms] | WCM [ms] |")
    print("| ----------- | -------- | ----- | ---- | ----- | ------ | ---- | -------- | -------- |")
    for file in input_files:
        ...
        print(f"| {file} | -       | {crc} | {size} | {get_human_readable_large_number(lines)} | {silver} | {gold} | {ccm:.2f} ± {ccstd:.2f} | {wcm:.2f} ± {wcstd:.2f} |")
    print("")
    ...
    print("Cold-cache:")
    print("```bash")
    print("hyperfine --input input.txt --prepare 'sync; echo 3 | sudo tee /proc/sys/vm/drop_caches' ./solution")
    print("```")
    print("Warm-cache:")
    print("```bash")
